...
export default function PostCard({ post }) {
    ...
    const initials = post.initials || (post.firstname ? `${post.firstname[0]}${post.lastname[0]}` : '??');
    const timeStr = post.time || new Date(post.created_at).toLocaleDateString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
