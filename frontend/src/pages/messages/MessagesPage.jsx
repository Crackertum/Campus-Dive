...
import { Send, Paperclip, Search, MessageSquare, CheckCheck } from 'lucide-react';
                    ...
                    <div className={`flex-1 flex flex-col ${activeThread ? 'flex' : 'hidden sm:flex'}`}>
                        {activeThread && otherUser ? (
                            <>
                                        ...
                                        return (
                                            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                                                <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${isMine
                                                    ...
                                                    {msg.attachment_path && (
                                                        <a href={`/${msg.attachment_path}`} target="_blank" rel="noopener" className="flex items-center gap-1 mt-1 text-xs underline opacity-75">
                                ...
                                {/* Message Input */}
                                <form onSubmit={sendMessage} className="p-4 border-t border-surface-100 dark:border-surface-800">
                                    <div className="flex items-center gap-3">
                                        <input
                                            ...
                                            disabled={sending}
                                        />
                                        <button type="submit" disabled={sending || !newMessage.trim()} className="btn-primary px-4 py-3">
