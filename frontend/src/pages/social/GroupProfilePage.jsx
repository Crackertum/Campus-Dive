...
export default function GroupProfilePage() {
    ...
    const fetchGroupData = async () => {
        ...
        try {
            ...
            // If group found, fetch posts
            if (res.data) {
                const postsRes = await socialApi.getGroupPosts(res.data.id);
                ...
                <aside className="space-y-8">
                    ...
                    <div className="card p-6 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0B1120]">
                        ...
                        <div className="flex flex-wrap gap-2">
                             {[...Array(6)].map((_, i) => (
                                <div key={i} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-white/5 flex items-center justify-center text-[10px] font-black text-slate-400 border border-slate-200 dark:border-slate-800">
                                    ?
                                </div>
                             ))}
                        </div>
